...
def compute_soft_cross_entropy(labels_multiannotator: np.ndarray, pred_probs: np.ndarray) -> float:
    ...
    empirical_label_distribution = np.zeros((N, num_classes))
    for i in range(N):
        # Average labels across annotators for each class
        empirical_label_distribution[i, :] = np.mean(labels_multiannotator[i, :, :], axis=0)
    ...
    return np.mean(soft_cross_entropy)

# _set_fine_search_range remains the same as in your original code
...
def find_best_temp_scaler(
    ...
    def softmax(x, temperature=1.0, axis=None, shift=True):
        if shift:
    ...
    for i, curr_temp in enumerate(coarse_search_range):
        ...
        soft_cross_entropy_coarse[i] = compute_soft_cross_entropy(labels_multiannotator, scaled_pred_probs)
    ...
    for i, curr_temp in enumerate(fine_search_range):
        scaled_pred_probs = softmax(log_pred_probs, temperature=curr_temp, axis=1, shift=False)
        soft_cross_entropy_fine[i] = compute_soft_cross_entropy(labels_multiannotator, scaled_pred_probs)
